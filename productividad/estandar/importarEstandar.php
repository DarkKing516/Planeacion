...
if (isset($_FILES['csvFileInput']) && $_FILES['csvFileInput']['error'] == UPLOAD_ERR_OK) {
    ...
    $insert_sql = "INSERT INTO estandar (cod_grupo, grupo, cod_proceso, proceso, cod_articulo, articulo, peso, nombre_estandar, t_estandar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $insert_stmt = $conn->prepare($insert_sql);
    $insert_stmt->bind_param("sssssssss", $cod_grupo, $grupo, $cod_proceso, $proceso, $cod_articulo, $articulo, $peso, $cod_estandar, $nombre_estandar);
    ...
    foreach ($lines as $line) {
        ...
        list($cod_grupo, $grupo, $cod_proceso, $proceso, $cod_articulo, $articulo, $peso, $cod_estandar, $nombre_estandar) = $data;

        // Verificar si el código ya existe en la base de datos
        $codigo_check = $cod_grupo;
        $check_stmt->execute();
        $check_stmt->store_result();

        if ($check_stmt->num_rows > 0) {
            echo "CÓDIGO DUPLICADO ENCONTRADO: $cod_grupo\n"; // Mensaje de depuración
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="importarEstandar.css">
</head>
<body>
...
<a href="estandar.php">
    <button>
        IR A GESTIÓN DE ESTÁNDAR
    </button>
</a>

<h1>GESTIÓN DE ESTANDAR.</h1>
...
<script type="text/javascript" src="importarEstandar.js" defer></script>
